...
func controlOutletTool() mcp.Tool {
	return mcp.NewTool("control_outlet",
		mcp.WithDescription("Set an outlet to ON, OFF, or AUTO. Use the outlet ID from get_outlet_states. AUTO returns the outlet to Apex program control. Use with care — this directly controls aquarium equipment."),
		mcp.WithString("id",
			mcp.Required(),
